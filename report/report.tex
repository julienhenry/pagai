...
satefy critical systems, such as avionics.
...
program. The state of a program is defined by the position where we are in the
code, and the actual values of the different numerical variables.
The set of possible states is expressed as a least fixpoint of a set of
equations defining the semantic of the program. 
...
the possible states during execution), either it is incomplete (it contains
states that can never be reached during execution). 
Abstract Interpretation is incomplete, since it always computes an
...
integers.
In this report, we present a novel refinment of the Abstract Interpretation
technique, that uses SMT-solving to guide the fixpoint iterations such as it
temporarily focus on a selected path in the control flow graph (the graph
expressing the semantic of the program), in order to
...
compute this number of path at the beginning of the algorithm.
...
analysis, but for code generation. Then, some informations we need to do precise
...
for instance, if $n$ is unsigned, one could add the constraint $n \geq 0$ in our
 ...
 \section{Abstract domain representation}
		...
		The analyzer is implemented with the APRON library \cite{JM09}, which
