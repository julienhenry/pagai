\documentclass[a4paper,english,titlepage,11pt,twoside]{report}
...
program.
Instead of convex polyhedra, one could choose intervals, octagons, etc, that are
subclasses of the class of convex polyhedra.
...
on the right hand side, until convergence.
...
disconnects every cycles. For instance, in structured programs, $P_w$ can be chosen as the set of the
loop headers.
...
Acceleration is possible when the loop is simple enough, and comes to computing
...
\ref{runningexample}. The set of control state where we apply widening is $\{p_1\}$: it
...
\item Step 2:
	...
	equal to $51$, so we add the path to
...
and then to be more precise. 
 ...
 \section{Path Focusing}
	...
	In this section, we present a refinement of Abstract Interpretation, refered
	as \emph{Path Focusing} technique \cite{MG11}.
...
\FloatBarrier
	...
	semantics $$\tau_{p_1 \rightarrow \dots \rightarrow p_k} = \tau_{p_{k-1}
	\rightarrow p_k} \circ
	\dots \circ \tau_{p_1 \rightarrow p_2}$$ where $\tau_{p_i \rightarrow
	p_{i+1}}$ is the transformation associated to the semantics of $p_i
	\rightarrow p_{i+1}$.
	\end{itemize}
...
path.
	...
	$\rho$ expressing the semantics of the program. For doing so, all the
...
in the $\rho$ formula.
\item For each operation in the program, we encode its semantics, expressed in
terms of the Boolean and numerical SSA variables. For instance, if the CFG
contains the assignment $x.2 = x.1 + 1$, then we simply add $x.2 = x.1 + 1$ to
...
\end{algorithm}
	...
	self loops of the multigraph. Indeed, instead of simply applying the widening
	...
	\textit{SelfLoop}$(\tau_e^\#,X_{p_i})$, defined in algorithm \ref{loopiteralgo}.

	The \textit{SelfLoop} procedure actually computes a widening operation, and
...
%DM: du bloc/fonction/truc
	...
	loop, i.e compute the invariants for a loop having its semantics
	...
	disjunct is the semantics of a path from $p$ to itself. It is of the form
	$$\bigvee_{1 \leq i \leq n} \tau_{p,i}$$ where $n$ is the number of
	paths, and $\tau_{p,i}$ is the semantics of the $i$-th path from $p$ to
	itself.
	\end{definition}
...
Computing disjunctive invariants is a field we have to work on. The work
described here is only preliminary, and shows it is possible to improve existing
techniques with our ideas, so that we avoid explicit enumeration of
exponential-sized sets of paths.
...
\begin{itemize}
\item the \emph{PromoteMemoryToRegister} Pass (-mem2reg), that transforms the internal
...
\item the \emph{LoopSimplify} (-loopsimplify) and \emph{LoopInfo} (-loopinfo) Passes, that transform all loops into a canonical
format, and compute some information related to loops (for instance, the set of
BasicBlocks that are parts of the loop, etc.).
\item the \emph{LowerSwitch} Pass (-lowerswitch), that transform a switch instruction into its
...
computing these live variables after the transformation in SSA form.
 ...
 \section{Abstract domain representation}
	...
	Morally, this means the $\Phi$-variables are on incoming transitions,
	whereas the other instructions are on outgoing transitions.
...
represent the set of possible states of our program, we lose information when
...
a linearization technique. However, multiplication is not allowed in LIA
...
\end{itemize}
	...
	subtle, the result at the end may be worse. This is a well-known result in
	abstract interpretation: when using widening operations, the final result is
	not monotonic with the quality of the abstraction.
...
the two methods, and get the best of both worlds (see \ref{combining}).
\end{itemize}
...
\chapter{Conclusion}
	...
	techniques.
	Abstract Interpretation is a commonly used technique to do static analysis of
	...
	well as real-life programs. It loses
