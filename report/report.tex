...
\abstract{
	Program verification is aimed at statically discover properties on programs,
...
\resume{  
	...
	Ce report s'inscrit dans la lignée des travaux visant à améliorer la
...
Static analysis is aimed at automatically computing properties on programs, such as
...
is aimed to decide the satisfiability of a logic formula containing Boolean
...
= \gamma \circ \Phi^\#(x)$. Abstract interpretation technique is aimed at computing
...
cases, $n$ can be choosen equals to $2$.
...
can be chosen, and the widening operator $\nabla_T$ is defined as follows:
$P \nabla_T Q$ is the intersection of $P \nabla Q$ with all the inequalities in
...
widening (which gives the result $P \nabla Q = \{(x,y)\ |\ 0 \leq x \leq y \}$),
a widening with the threshold $x \leq 50$ gives 
$P \nabla_T Q = \{(x,y)\ |\ 0 \leq x \leq y \leq 50\}$. In such cases, widening
...
with a sequence of $n$ \emph{if-then-else} between point $p_1$ and $p_n$, 
the $n$ merge node
...
\end{algorithm}
	...
	before focusing on another path. This is the same principle as the one we
	find in \emph{Lookahead widening} \cite{GopanR06}.
...
easily compute this number of paths when computing the $\rho$ formula.
	...
	during the acending sequence, but during narrowing iterations that occur 
...
possible. The technique is then to stabilize the iterations over this subgraph,
removing $p_4$ and $p_6$ for a while. After stabilization and narrowing phases,
...
two different polyhedron to the BasicBlock. However, we prefered the following
...
implemented in our analyzer, and this transformation has to be done manually.
...
In our analyzer, this kind of loops gives imprecise results. In this section, we
...
In our analyzer, we unroll every loops once, by using LLVM optimisation
passes such as \emph{loop-unroll}, \emph{loop-rotate} or \emph{loop-simplify}.
	...
	Our analyzer could be extended in many ways. Indeed, it is intra-procedural,
	...
	Additionnaly, our analyzer has no memory model:
\begin{itemize}
...
analyzer considers $x$ can take all values of its type.
\end{itemize}
...
Additionally, some optimization in the code could be proposed in order to
reduce time and/or memory complexity. For instance, paths that have already been
computed are until now stored in a tree. An optimization could be to store them
...
\end{itemize}
 ...
 \section{Experiments}
	...
	gives the results of the analyzer in various examples, some of them taken
...
for reading the program, initializing the SMT-solver, \dots, may have a
...
relations into the formula. Then, it was a good choice to link our analyzer with
both SMT-solvers.

\begin{table}[!h]
\centering \scriptsize
\begin{tabular}{|c|c|c|l||c|l|} \hline
...
Boustr.c & 0.100s & 0.088s & \begin{tabular}{l}
 ...
 -2x.0 + 3d.0 + 2001 >= 0 \\
 -d.0 + 1 >= 0
\end{tabular} & 0.064 & \begin{tabular}{l}
...
-i.0 + 100 >= 0 \\
i.0 >= 0
\end{tabular} & 0.052 & \begin{tabular}{l}
-i.0 + 100 >= 0\\
r.0 >= 0 \\
i.0 - r.0 >= 0 \\
24i.0 - 25r.0 + 75 >= 0
...
ex8.c & 0.516s & 0.408s & \begin{tabular}{l}
 ...
 y.0 - x.0 + 1 >= 0\\
 y.0 + x.0 + 1 >= 0
\end{tabular} & 0.208 & \begin{tabular}{l}
-x.0 + 1 >= 0 \\
x.0 + 1 >= 0 
\end{tabular} \\ \hline
...
b.0 - c.0 + 1 >= 0\\
b.0 + c.0 + 1 >= 0\\
\end{tabular} & 0.916s & \begin{tabular}{l}
-b.0 + 1 >= 0\\
-b.0 + a.0 + 1 >= 0\\
-c.0 + 1 >= 0\\
-c.0 + d.0 + 1 >= 0\\
...
b.0 - c.0 + 1 >= 0\\
b.0 + c.0 + 1 >= 0\\
\end{tabular} & 36.300s & \begin{tabular}{l}
-b.0 + 1 >= 0 \\
-b.0 + a.0 + 1 >= 0\\
-c.0 + 1 >= 0\\
-c.0 + d.0 + 1 >= 0\\
...
\chapter{Conclusion}
	...
	Validation of softwares has become an essential domain in industry,
	...
	analyzer, in order to distinguish the different paths of the program
	and to treat them
	...
	We implemented this technique into a small analyzer dealing with C and C++
	...
	Our analyzer is able to deal with small- and middle-sized programs, but
	looses
