...
To do so, one could implement an LLVM pass that transform the internal
...
\end{itemize}
 ...
 \section{Experiments}
	...
	Focusing} implementations. We can compare their execution time in some
...
Gopan.c & 0.088s & 0.080s & \begin{tabular}{l}
-x.0 - y.0 + 102 >= 0 \\
y.0 >= 0              \\
x.0 - y.0 >= 0        
...
ex8.cpp & 0.516s & 0.408s & \begin{tabular}{l}
 -y.0 - x.0 + 1 >= 0\\
 -y.0 + x.0 + 1 >= 0\\
 y.0 - x.0 + 1 >= 0\\
 y.0 + x.0 + 1 >= 0
...
ex8c.c & 191m50.935s & $\infty$ & \begin{tabular}{l}
c.0 + a.0 = 0 \\
b.0 + d.0 = 0\\
-b.0 - c.0 + 1 >= 0\\
-b.0 + c.0 + 1 >= 0\\
b.0 - c.0 + 1 >= 0\\
b.0 + c.0 + 1 >= 0\\
\end{tabular} & 0.928 & \begin{tabular}{l}
-b.0 + 1 >= 0\\
-b.0 + a.0 + 1 >= 0\\
-c.0 + 1 >= 0\\
-c.0 + d.0 + 1 >= 0\\
...
techniques applied to various benchmarks}
\label{experiments}
\end{table}
...
between all the paths of the loop.

To be completed\dots

\begin{table}[!h]
\begin{tabular}{|l||c|c|c|c|c|c|c|} \hline
Benchmark  & Gopan.c & Beyer.c & Boustr.c & ex8.cpp & ex9.cpp & test1.c & ex6.c\\ \hline \hline
paths (total) & 5 & 4 & 10 & 75626 & 8 & 3 & 5 \\ \hline
paths (computed) & 5 & 4 & 4 & 4 & 4 & 3 & 4\\ \hline
iterations  & 6 & 5 & 5 & 4 & 4 & 3 & 4\\ \hline
\end{tabular}
...
\chapter{Conclusion}
	...
	Our technique performs better because of the distinction between
	paths. Then, encoding some properties, such as arithmetic overflows or
	aliases, into distinct paths in the control flow graph is one direction to
	explore.

	Our analyzer is able to deal with small- and middle-sized programs, but lose
