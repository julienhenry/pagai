...
The least fixpoint of a function $\Phi^\#$ is noted $\overline{x}$ and is equal
to $\displaystyle \lim_{i \geq 0}\ x_i$, where $x_0 = \perp$ (the least element
of $A$) and $x_{i+1} = \Phi^\# (x_i)$. Instead of computing this least fixpoint,
...
\FloatBarrier
	...
	First, one choose a set of points $P_W \subseteq P$, so that the graph
...
compute these live variables after the transformation in SSA form.
 ...
 \section{Abstract domain representation}
	...
	there would be a huge memory consumption if we would attach a polyhedra
