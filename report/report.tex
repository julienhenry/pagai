...
the formula.
\item For each transition $(p_i,p_j) \in E$, we set $t_{i,j} = c(i,j)$, where
...
$b_i = \displaystyle \bigvee_{(p_j,p_i)\in E} t_{i,j}$.
\item For each point $p_i^d \in P_R$, we set the reachability predicate 
$b_i^d = \displaystyle \bigvee_{(p_j,p_i)\in E} t_{i,j}$. 
\item $\Phi$-variables are treated differently, depending on the control point
$p$ where they are defined. To be completed !!
