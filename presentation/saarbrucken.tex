...
\title[Path-focusing technique]{Path-Focusing technique}

\subtitle{Improving precision of Abstract Interpretation} % (optional)
...
\section[Combined technique]{Combining Both Techniques}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Apron Library for the abstract domains
		\item Microsoft Z3, Yices (SMT Solvers)
	\end{itemize}
\end{frame}
...
\section{WCET by Path-focusing}

\begin{frame}
	\frametitle{Computing WCET by Path-focusing}
Path Focusing:
	\begin{columns}
		\begin{column}{4cm}
\begin{tikzpicture}[->,>=stealth',auto,node distance=1.1cm,
...
\begin{frame}
	\frametitle{Computing WCET by Path-focusing}
	\begin{itemize}
		\item We attach a cost $c_k$ to each BasicBlock $b_k$
		\item $cost = \displaystyle \sum_{k} b_k~c_k$
	\end{itemize}
	
	$$\rho \wedge \left(cost = \sum_{k} b_k~c_k\right)$$
	\vspace{1cm}
	We compute the maximal cost by dichotomy.
\end{frame}

\begin{frame}
	\frametitle{Computing WCET by Path-focusing}
	Each time we check the formula, the SMT-solver
gives us a value for the cost if the problem is satisfiable.
\begin{itemize}[<+->]
\item Start with a maximal bound (here, 100) 
\item Check 1: $cost>50$, get $cost = 70$
\item Check 2: $cost>85$, get no
\item Check 3: $cost>77$, get $cost = 82$
\end{itemize}
\vspace{1cm}
\begin{center}
\begin{tikzpicture}
	\node at (10,0) [yshift=-0.2cm] {$100$};
	\node at (0,0) [yshift=-0.2cm] {$0$};
	\node<2-3> at (7,0) [yshift=-0.2cm] {$70$};
	\node<3-> at (8.5,0) [xshift=0.2cm,yshift=-0.2cm] {$85$};
	\node<4-> at (8.2,0) [xshift=-0.2cm,yshift=-0.2cm] {$82$};

\fill[green] (0,0) rectangle  (10,0.5);
\fill<2->[red] (0,0) rectangle  (7,0.5);
\fill<3->[red] (8.5,0) rectangle  (10,0.5);
\fill<4->[red] (0,0) rectangle  (8.2,0.5);
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}
	\frametitle{Problem}
	\begin{block}{Problem}
		Time blowup near the WCET
	\end{block}

	The SMT solver does not remember some basic inequalities about the cost of
	a portion of the CFG.

	\vspace{1cm}

	\visible<2->{We add these basic inequalities during the binary search.}
\end{frame}

\begin{frame}
	\frametitle{Another approach\ldots}
	\begin{block}{Idea}
		Compute the WCET by Abstract Interpretation (Path Focusing)
	\end{block}

	\vspace{1cm}

	We could instrument our LLVM bitcode with:
	\begin{itemize}
		\item 1 counter per BasicBlock
		\item OR 1 single general counter
	\end{itemize}

	We obtain a WCET which may depend on the function parameters, etc\ldots
\end{frame}
...
\begin{frame}
	...
	For computing WCET:
	\begin{itemize}[<+->]
		\item Path Focusing: avoid unfeasible paths
		\item Disjunctive invariants: more expressive abstract domain
		\item Instrument the bitcode ?
	\end{itemize}
\end{frame}
\end{document}
