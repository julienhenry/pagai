...
\begin{column}{5cm}
$i \in
[0,\only<1>{0}\only<2>{1}\only<3>{2}\only<4>{3}\only<5>{+\infty[}\only<-4>{]}\only<6>{100]}$

\bigskip

\only<2-5>{ascending iterations}
\only<6>{descending iterations}
\end{column}
\end{columns}
\end{frame}
...
\bigskip
\begin{itemize}
\item separate loops into several phases.
...
\end{itemize}
\bigskip
``Does it exists a path starting in our invariant candidate, that arrives in a
state outside of our invariant {?}`` 
\bigskip
...
\begin{itemize}
\item work on the Path-focusing technique
