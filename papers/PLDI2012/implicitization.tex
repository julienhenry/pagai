...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \WHILE{true}
    \STATE Ascending iteration on a \textit{Subset} (lines~\ref{alg=start-ascending}-\ref{alg=end-ascending} in algorithm~\ref{alg=combined})
    \STATE Narrow (line~\ref{alg=narrowing} in algorithm~\ref{alg=combined})
    \STATE Add new paths to \textit{Subset} (lines~\ref{alg=start-add-paths}-\ref{alg=end-add-paths} in algorithm~\ref{alg=combined})
    \ENDWHILE
  \end{algorithmic}
\end{algorithm}
