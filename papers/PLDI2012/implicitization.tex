...
compact representation, such as Algebraic Decision Diagrams.\MM{Du
  coup, j'aurais intégré cette phrase au paragraphe suivant. C'est des
  ADD que tu utilises finalement ?}
...
\MM{Et que prend-t-on pour $\delta$ ? C'est 1 tout le temps ?}
...
\begin{figure}[h]
  \MM{Je trouve que les zéro-virgule-quelque-chose comme .17 font
    moches à côté des zéros, vu que les tableaux sont alignés sur le
    ., les ``0'' sont tout à gauche et les ``.17'' tout à droite. Je
    vote pour des 0.17 pour que l'alignement soit bon, ça rendrait la
    lecture plus facile je trouve}
  \begin{center}
...
\MM{Pourquoi cet item se trouve dans la section ``Precision of the
  various techniques'' et pas dans la section précédente sur
  l'implem ?}
...
algorithm.
\MM{Si y'a la place, on peut dire un truc du style: ``This can be
  explained by the fact that these algorithms start on a small subset
  of the graph, and then may need fewer iterations on the complete
  graph.''}
...
using the classical widening operator (POLY).}
\label{tab:domain}
\end{table*}
...
\MM{Oui, ça fait un peu [citation needed] comme phrase ;-)}
