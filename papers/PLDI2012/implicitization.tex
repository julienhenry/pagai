...
\preprintfooter{Succinct representations for abstract interpretation}   % 'preprint' option specified.
...
suivant)}
We start with one single
disjunct per control point ($m=1$) and an empty map $\sigma$.
...
disjunct of index $k'$ so that the least upper bound of the two abstract values is exactly their union (this can be tested using SMT-solving).
If such an index exists, then we set $\sigma(j,k) = k'$.
Otherwise:
...
\DM{Julien: pour la guided analysis, a priori ça arrive souvent cette histoire d'edge pas prise car non encore prenable?}
\JH{Oui, ca arrive regulierement}
...
\JH{On peut. C'est PKGRID qui ne fonctionne pas}
