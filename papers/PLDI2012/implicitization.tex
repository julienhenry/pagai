...
\MM{Je crois que les anglophones utilisent plutôt $99990)$, mais
  d'après Wikipedia ``Interval (mathematics)'', les deux sont acceptés
  par le standard ISO}
...
associated to a control point $p_j$, noted $X_j^d$, which is distinct
from $X_j^s$, and initialized to $X_j^s$. This value is updated when a
point $p_j$ is the target of a new path, and is used instead of
$X_j^s$ in the SMT formula. This way, further SMT queries do not
...
Here, only 3 paths out of the 25 have been computed during the analysis. In
...
$\bigvee_{1\leq j \leq m} X_{i,j}$. We denote by $n$ the number of
distinct paths in the loop.
To perform the analysis, one chooses an integer $\delta \in [1,m]$, and
a mapping function $\sigma: [1,m] \times [1,n] \mapsto [1,m]$.
\MM{C'est moi ou le $\tau_{i,k}$ ne correspond pas exactement à la
  notation du début ? Au début, $\tau_{i,k}$ est la transition de $i$
  à $k$, alors que là, on est dans le multigraphe donc il y a
  plusieurs arrêtes entre deux points. Si je comprends bien,
  $\tau_{i,k}$ est le $k$-ième chemin partant de $i$, c'est ça ?
  Je mettrais bien ``... and for the $k$-th path starting from $i$,
  denoted $\tau_{i,k}$, the image...'' si c'est bien ça. D'ailleurs,
  faut-il forcément parler de boucle ici ? La technique ne s'applique
  pas à des chemins quelconques ?}
  For each
...
$X_{i,\sigma(j,k)}$.
Initially, the $\delta$-th abstract value contains the initial states of
...
Note that $\sigma(j,k)$ will be computed only if needed.

\MM{Je trouve ça assez frustrant d'arriver ici et de ne toujours pas
  savoir quel $\sigma$ est choisi. A mon avis, on peut remonter la
  description du $\sigma$ dynamique de la section suivante à ici. Ça
  évite de le présenter comme un petit détail d'implémentation alors
  que c'est quand même une vraie contrib par rapport au papier
  original.}
...
Abstract domains are provided by the APRON library \citep{DBLP:conf/cav/JeannetM09}, and include convex polyhedra, octagons, intervals, and linear congruences.
...
\MM{cf. ci-dessus, j'aurais mis ça dans la section invariants
  disjonctifs}
...
exactly the union of them. If such an index exists, then we set $\sigma(j,k) = k'$.
Otherwise:
\begin{itemize}
...
algorithm. The techniques we introduce in this article are slower, but speed is still on the same order of magnitude as the older ones.
...
commonly yields greater invariants than the domains of octagons/intervals;
this is a known weakness of its widening operator.
% DM Ici on pourrait citer Monniaux+Le Guen si c'etait publie!
The Octagon domain appears to be much better than intervals.
...
\DM{Je croyais qu'on ne pouvait pas utiliser PPLGRID?}
