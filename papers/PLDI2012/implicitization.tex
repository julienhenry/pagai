...
self loops, and \emph{Path-focusing} only performs a widening. 
...
We also unroll the loop once, in order to distinguish the paths that don't go
...
For instance, one could defined $\sigma$ so that $\sigma(j,k)$ only depends on the
