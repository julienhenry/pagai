...
$b_i^d$ is $true$ when the path arrives at $p_i$.
...
\citet{DBLP:conf/pldi/GulwaniZ10} proposes a technique for computing disjunctive invariants, by
...
Our improvement consists in a slight modification of the SMT formula we solve.
We introduce in this formula Boolean variables $\{d_k, 1 \leq k \leq m\}$, so
that we can easily find in the model which abstract value of the disjunction we
have to choose. The resulting formula that is given to the SMT solver is defined
by $g(p_i)$.
...
\WHILE {true}
	...
	\STATE $X_{i,\sigma(j,k)} \gets X_{i,\sigma(j,k)} \sqcup
	\tau_{i,k}(X_{i,j})$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
