...
The concrete and abstract domains are connected by a monotone \emph{concretization} function $\gamma: (\abstr{X},\sqsubseteq) \rightarrow (\parts{X},\subseteq)$: an element $\abstr{x} \in \abstr{X}$ represents a set $\gamma(\abstr{x})$.%
%
\footnote{%
...
\emph{Guided static analysis} was proposed by \citet{DBLP:conf/sas/GopanR07} as an improved over classical upward Kleene iterations with widening.
...
graph of the program. One could instead apply the same technique to the implicit
...
We also assume that the program in expressed in SSA form, meaning that each
variable is only assigned once in the transition graph. This is not a
restriction, since there exist standard algorithm that transform a program into
an SSA form.
...
expressed within a decidable theory.
...
The algorithm for computing an inductive invariant over a subgraph is the Path
Focused algorithm from \citet{Monniaux_Gonnord_SAS11}.
...
\in P_R$ of $p_i$, such that the image of $X_{p_i}^s$ by the path transformation
is not included in the actual $X_{p_j}^s$.
Intuitively, such a path makes the abstract value $X_{p_j}^s$ grow, and thus is
...
have to cunjunct the formula $f(p_i)$ with the actual set of working paths,
...
to give us a path which is in $P$.
...
\item  When the precedent phase terminates, there is no more paths in
$P$, starting at $p_i$, 
...
computes these paths and adds them to $P'$.
\end{enumerate}

It is not necessary to add into $P'$ all the new feasible paths. We just have
to add a new path into $P'$ until no more path can make the invariant grow,
...
\citet{DBLP:conf/sas/GopanR07}, we can do some narrowing iterations, and we
restart the analysis with a new subgraph if needed.
\begin{algorithm}
	\caption{Guided static analysis on implicit multigraph}
	\label{algo:combined}
	\begin{algorithmic}[1] 
	\input{algo/algo-combined}
	\end{algorithmic}
\end{algorithm}
