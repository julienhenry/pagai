...
\newcommand{\JH}[1]{{\color{blue} TODO(JH): #1}}
...
Such iterations necessarily lead to a stationary $(\abstr{x}_p)_{p \in P}$, which define an inductive invariant $(\gamma(\abstr{x}_p))_{p \in P}$. Note that this invariant is not, in general, the least one expressible in the abstract domain, and may depend on the iteration ordering (the successive choices~$p,p'$).
...
\begin{enumerate}
	\item The first non-empty subset of paths is depicted on Figure
		...
		We also compute the set $P'$ of paths that have to be added at the next
		iteration. 
		For describing the different paths in the loop, we introduce the Boolean
		values $c_7, c_8, c_9$, where $c_i$ is \emph{true} when the path goes
		through the \emph{then} branch of the \emph{if-then-else} at line $i$, and
		\emph{false} when the path goes through the \emph{else} branch.
		We then add into $P'$ the following paths:
\JH{A compl\'eter}
\end{enumerate}
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.1cm,
  ...
  \path [transition] 
		(n00) edge  node {} (n0);
  \path [transition] 
		(n0) edge  [loop right] node [right] {$\neg c_7 \wedge c_8 \wedge \neg c_9$} (n0);
  \path [transition] 
		(n0) edge  [loop left] node [left] {$\neg c_7 \wedge c_8 \wedge \neg c_9$} (n0);
  \path [transition] 
		(n0) edge  [out=-45, in=45] node [right] {$\neg c_7 \wedge \neg c_8 \wedge c_9$} (n1);
  \path [transition] 
		(n0) edge  [out=-135, in=135] node [left] {$\neg c_7 \wedge \neg c_8 \wedge c_9$} (n1);
\end{tikzpicture}
\end{minipage}
