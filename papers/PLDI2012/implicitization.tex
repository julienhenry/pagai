...
\MM{C'est moi ou le $\tau_{i,k}$ ne correspond pas exactement à la
  ...
  The image of the $j$-th disjunct $X_{i,j}$ by the path $\tau_{i,k}$ is joined with 
  $X_{i,\sigma(j,k)}$.
...
value, and the path we focus on. If we dynamically construct these functions
during the analysis, we should store the results of the $\sigma_i$ functions into a
compact representation, such as Algebraic Decision Diagrams.
Note that $\sigma_i(j,k)$ will be computed only if needed.
...
disjunct per control point ($m_i=1$ for each $i, p_i \in P_R$) 
and empty maps $\sigma_i$.
$M$ denotes an upper bound on the number of disjuncts per control point.
$\sigma_i(j,k)$ is then constructed on the fly only when needed.
When $\sigma_i(j,k)$ needs to be computed, we first compute the image of the
