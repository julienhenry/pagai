...
if $\abstr{\tau}_{p,p'} (\abstr{x}_p) \not\sqsubseteq \abstr{x}_{p'}$, replace $\abstr{x}_{p'}$ by $\abstr{x}_{p'} \sqcup \abstr{\tau}_{p,p'} (\abstr{x}_p)$.
Such a stationary limit is bound to exist if $\abstr{X}$ has no infinite ascending chain $a_1 \sqsubset a_2 \sqsubset \dots$; this condition is however not met by domains such as intervals or convex polyhedra.

\emph{Widening-accelerated Kleene iterations} proceed by replacing $\abstr{x}_{p'} \sqcup \abstr{\tau}_{p,p'} (\abstr{x}_p)$ by $\abstr{x}_{p'} \widening (\abstr{x}_{p'} \sqcup \abstr{\tau}_{p,p'} (\abstr{x}_p))$ where $\widening$ is a \emph{widening operator}: for all $\abstr{x},\abstr{y}$, $\gamma(\abstr{y}) \subseteq \gamma(\abstr{x} \widening \abstr{y})$, and any sequence $\abstr{u}_1,\abstr{u}_2,\dots$ of the form $\abstr{u}_{n+1} = \abstr{u}_n \widening \abstr{v}_n$, where $\abstr{v}_n$ is another sequence, become stationary.
The stationary limit $(\abstr{x}_p)_{p \in P}$, defines an inductive invariant $(\gamma(\abstr{x}_p))_{p \in P}$. Note that this invariant is not, in general, the least one expressible in the abstract domain, and may depend on the iteration ordering (the successive choices~$p,p'$).
...
Consider the following program, adapted from \cite{Monniaux_Gonnord_SAS11}, where \lstinline|input($a$, $b$)| stands for a nondeterministic input in $[a,b]$ (the control-flow graph on the right depicts the loop body, $s$ is the start node and $e$ the end node):
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=.2cm and .5cm,
	...
	\node[PRstate] (n9) [below right=of n7] {$e$};
...
From there, the ``else'' branch at line~4 may be taken; with further widening, $0 \leq y \leq x$ is obtained as a loop invariant, and thus the computed loop postcondition is $x \geq 0 \land y = 0$.
...
$I_p$ denotes a set of initial states at program point~$p$ (thus $\emptyset$ for most $p$).
%
...
Each time the invariant candidate of a point $p_j$ has been updated, $p_j$ is
inserted into $A'$ since it may be the start of a new feasible paths.
...
The new paths in $P$ are computed one after another, until no more path
...
SMT solver: we now try to find a path that is not yet in $P$, but is feasible
...
%\vspace{-1em}
\]
$X_j$ is updated using an abstract union when the point $p_j$ is the target of a new path. This way, further SMT queries do not
compute other paths with the same source and destination if it is not needed
(because these new paths would not make $X_j$ grow, hence would not be returned
by the SMT solver).
...
When a new path has been found, it is immediately added into $P$. 
We then have to add $p_i$ and $p_j$ into $A$ (since we do not apply widening in
this section) and $p_j$ into $A'$, since $p_j$ may be the starting point of a
new feasible path.
...
\begin{inparaenum}
\item 
the subset of paths $P$ strictly increases at each loop iteration,
and is bounded by the finite set of
paths in the entire graph. 
\item
	when computing new paths, we cunjunct our formula with $\neg P$, meaning
...
finds $\lstinline|x_old| \in (-\infty,+\infty)$.
...
\begin{compactenum}
	...
	\item
		...
		subgraph. We first find the path from $p_1$ to $p_3$ and obtain at $p_3$
		$\lstinline|x_old| = 0$.
                \MM{TODO: il y avait cette phrase pas finie `` We then
                  find the path''. J'ai enlevé. C'est bon ?}
		
		The image of $\lstinline|x_old| = 0$ by the path that goes from $p_3$ to
		$p_8$, and that goes through the \emph{else} branch of each 
		\emph{if-then-else}, is $-10 \leq \lstinline|x_old| \leq 10$. This path
		is then added to our subgraph. 
		
		Moreover, there is no other path starting at $p_3$ whose
		image is not in $-10 \leq \lstinline|x_old| \leq 10$.

		Finally, since the abstract value associated to $p_8$ is  
		$-10 \leq \lstinline|x_old| \leq 10$, the path from
                $p_8$ to $p_3$\MM{TODO: y'avait $p_1$ ici, j'ai mis $p_3$, c'est bon ?} is
		feasible and is added into $P$. The final subgraph is depicted on Figure
...
$-\infty \leq \lstinline|x_old| \leq +\infty$ for both $p_3$ and $p_8$.
	\item
		We now can apply narrowing iterations, and recover the precision lost by
		widening: we obtain 
		$-10000 \leq \lstinline|x_old| \leq 10000$ at points $p_3$ and $p_8$.
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.1cm,
	...
	\node[PRstate] (n00) {$p_1$};
	\node[PRstate] (n0) [below of=n00, yshift=0.8cm] {$p_3$};
	\node[PRstate] (n1) [below of=n0] {$p_{8}$};
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.1cm,
  ...
  \path [transition] 
		(n0) edge [bend left] node [left, yshift=-0.2cm, xshift=0] {
		$\begin{array}{r}
...
% JH: plus besoin de ca
%Still, we need to unroll
...
\begin{figure}[h!]%
  ...
  The $\subsetneq$ bars (resp. $\supsetneq$) gives the percentage of invariants stronger (more precise; smaller with respect to inclusion) with the left-side (resp. right-side) technique,
...
(iii) as suggested previously \cite{Colby_PhD,DBLP:conf/vmcai/Mine06}, this approach is more precise than running abstract operations for each program line separately:
...
result of the classical Abstract Interpretation in $2.21\%$ of the control points in $P_R$.
...
Static Analysis} in $5.00\%$ of the cases. However, it also loses
precision compared to \emph{Guided Static Analysis}  in an
important number ($4.00\%$) of control points.

Unfortunately, these two techniques are less precise than the classical analysis
in a important number of cases ($4.64\%$ for $G$, $5.14\%$ for $PF$).
This could be explained by the non-monotonicity of the results with widening operators.
...
precision of the analysis (for $14.46\%$ of the control points in $P_R$), at an
acceptable time cost compared to our combined algorithm (see Table \ref{tab:time}). \MM{TODO}
...
of \emph{Lapack/Blas}, where 668 over the 1498 functions have been skipped.
...
\emph{Path-focusing} technique is sometimes faster than the classical algorithm.
This seems due to the fact that this technique avoids many convex hulls, which
is costly, and also computes the image of an abstract value by an entire path
instead of many single transitions, thus leading to a large parallel assignment.
